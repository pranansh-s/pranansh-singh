...
const Hero: FC = () => {
  ...
  return (
    <section
      id="hero"
      className="relative flex h-screen items-center justify-center text-center text-white xl:snap-center"
    >
      <Canvas />
      <div className="grid h-max w-[26rem] grid-cols-3 items-center justify-items-end gap-x-3 md:w-[35rem] lg:w-[53rem] lg:gap-x-8 xl:w-[70rem]">
        <motion.h1
          initial={{ x: -200, opacity: 0 }}
          whileInView={{ x: 0, opacity: 1 }}
          transition={{ duration: 1 }}
          className="col-span-2 mr-auto text-left font-outerBold text-4xl uppercase drop-shadow-[0_0_4px_rgba(0,0,0,1)] md:text-5xl lg:text-7xl xl:text-8xl"
        >
          pranansh
        </motion.h1>
        <motion.sub
          ...
          className="hov mt-auto pb-1 font-outerMedium text-xs text-white/30 transition-colors duration-500 hover:text-white/70 md:text-sm lg:text-lg xl:text-xl"
        >
          clean work on time
        </motion.sub>
        <motion.sub
          ...
          dragElastic={0.03}
          transition={{ duration: 0.8, type: 'spring', stiffness: 100, delay: 0.5 }}
          className="hov w-36 font-cedarville text-sm text-white/30 transition-colors duration-500 hover:text-white/70 md:text-base lg:w-52 lg:text-xl xl:text-2xl"
        >
        ...
        <div className="hov relative mr-auto md:scale-125 lg:scale-[1.8]">
          ...
          >
            <motion.path
              ...
              animate={{ pathLength: 0.52, opacity: 1 }}
              transition={{ duration: 2 }}
              strokeWidth={4}
              d="M20.7433 63.9884C13.1188 70.3958 10.2626 81.1531 12.0573 91.1792C13.9745 101.884 21.2174 110.799 31.081 115.153C41.0416 119.551 52.5977 119.546
              ...
              64.5033 21.2922 63.5283 20.7433 63.9884Z"
              stroke="#FF5858"
            />
            <motion.path
              initial={{ pathLength: 0, opacity: 0 }}
              animate={{ pathLength: 0.22, opacity: 1 }}
              transition={{ duration: 0.5, delay: 1.4 }}
              strokeWidth={5}
              d="M120.644 27.3614C119.023 27.7556 117.98 30.038 117.11 31.3431C116.048 32.9358 114.99 34.5256 113.963 36.1433C112.917 37.789 111.875 39.4355 110.83 41.0812C109.768 42.7572 108.422 44.4386
              ...
              40.952C142.734 41.7442 144.126 39.6378 142.848 38.7395C139.635 36.4737 136.273 34.4352 132.963 32.327C131.36 31.3075 129.739 30.316 128.122 29.3217C126.701 28.4477 124.972 27.053 123.28 27.1604C123.087 27.1732 122.929 27.374 123.007 27.5754Z"
              stroke="#FF5858"
            />
          </motion.svg>
        </div>
        <motion.h1
          ...
          whileInView={{ x: 0, opacity: 1 }}
          transition={{ duration: 1 }}
          className="text-right font-outerBold text-4xl uppercase drop-shadow-[0_0_4px_rgba(0,0,0,1)] md:text-5xl lg:text-7xl xl:text-8xl"
        >
          singh
        </motion.h1>
      </div>
      <div className="lg:-right-18 absolute -right-8 bottom-12 flex rotate-90 items-center gap-1 opacity-40 md:-right-12 md:bottom-20 md:gap-3">
        <div className="h-[0.1rem] w-10 rounded-xl bg-white md:w-24" />
        <div className="font-outerRegular text-[0.35rem] md:text-[0.4rem] lg:text-[0.5rem]">Portfolio</div>
      </div>
      <motion.div
